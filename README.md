# 与原版的区别
- 新增英文键盘字体缩放选项，解决关闭"上滑选择符号"后，字体过小的问题。
- 根据个人习惯精简了符号/表情标签，并加大标签页尺寸；符号页面现在默认锁定；移除微信专有emoji逻辑。
- 根据个人习惯重排默认风格的键盘布局，现在切换语言后功能键位置更加一致。
- 添加了一个清除最近符号列表的按钮；去除了最近符号列表若为空则展示第一页的逻辑。
- 根据个人习惯，调整了候选词横条、列表的位置。给展开候选词列表、回退按钮添加了更多间距避免误触。
- 临时修复了符号侧标栏的崩溃问题。
- **重要**：添加日语输入方案，语言键现在会在中/英/日三间循环（自绘了新的语言键图标）。

## 日语输入法
感谢原作者：https://github.com/gkovacs/rime-japanese

- 基于原作者的方案，将`っち`的拼写，从`tch`统一改成`cch`。
- 同时简化了schema的定义，移除了android端用不到的功能，比如反查、快捷键。详见 [japanese.schema.yaml](./src/main/assets/rime/build/japanese.schema.yaml)

## 想要添加其它词库？
将原始词典放入 `./src/main/assets/rime` 文件夹下。通过Android Studio运行程序。待rime build完成后，导出 `./src/main/assets/rime/build` 里的二进制文件。
